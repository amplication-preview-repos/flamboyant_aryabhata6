/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { AccountBalanceWhereInput } from "./AccountBalanceWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { AccountBalanceOrderByInput } from "./AccountBalanceOrderByInput";

@ArgsType()
class AccountBalanceFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => AccountBalanceWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => AccountBalanceWhereInput, { nullable: true })
  @Type(() => AccountBalanceWhereInput)
  where?: AccountBalanceWhereInput;

  @ApiProperty({
    required: false,
    type: [AccountBalanceOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [AccountBalanceOrderByInput], { nullable: true })
  @Type(() => AccountBalanceOrderByInput)
  orderBy?: Array<AccountBalanceOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { AccountBalanceFindManyArgs as AccountBalanceFindManyArgs };
